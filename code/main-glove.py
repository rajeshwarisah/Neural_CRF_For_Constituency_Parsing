    ...
    def inside(self, scores, mask):
        ...
        for d in range(2, seq_len + 1): # d = 2, 3, ..., seq_len
            ...
            if d == 2:
                ...
                s.diagonal(offset).copy_(scores.logsumexp(2).diagonal(offset))
            else:
                import pdb
...
#                 pdb.set_trace()
                ...
                s.diagonal(offset).copy_(s_span + scores.logsumexp(2).diagonal(offset))
...
class Model(nn.Module):
    def __init__(self, n_words, n_labels, n_tags, n_embed=50, n_feat_embed=100, embed_dropout=.33, 
                    n_lstm_hidden=400, n_lstm_layers=3, encoder_dropout=.33,
                    n_label_mlp=100, mlp_dropout=.33, embedding=None):

        super().__init__()
#         self.word_embed = nn.Embedding(num_embeddings=n_words, embedding_dim=n_embed)
        self.word_embed = torch.nn.Embedding.from_pretrained(torch.from_numpy(embedding).float())
...
def evaluate(model, loader):
    ...
    for words, *feats, trees, charts in loader:
        ...
        if False: ### draw trees here
            ...
            draw_tree(t, res_path="./drawing/" + str(i) + "_prediction" )

        if False: ### draw trees here
            filter_delete = lambda x: [it for it in x if it not in args.delete]
            trees_fact = [Tree.factorize(tree, args.delete, args.equal) for tree in trees]
            leaves = [filter_delete(tree.leaves()) for tree in trees]
            t = convert_to_viz_tree(tree=trees_fact[0], sen=leaves[0])

            draw_tree(t, res_path="./drawing/" + str(i) +  "_original")
...
if __name__ == "__main__":
    ...
    model = Model(n_words=args.n_words, n_labels=args.n_labels, n_tags=args.n_tags, 
                    n_lstm_hidden=args.n_lstm_hidden, n_lstm_layers=args.n_lstm_layers, embedding=embs_npa).to(args.device)
    print(model)
...
#     pdb.set_trace()
    ...
    print(evaluate(model, testdata.loader))
