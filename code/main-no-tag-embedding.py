...
#                 pdb.set_trace()
                ...
                s_s = stripe(s, n, offset - 1, (0, 1)) + stripe(s, n, offset - 1, (1, offset), 0)
                # [batch_size, n, w]
                s_s = s_s.permute(2, 0, 1)
                s_s.register_hook(lambda x: x.masked_fill_(torch.isnan(x), 0))
                s_s = s_s.logsumexp(-1)
                s.diagonal(offset).copy_(s_s + scores.logsumexp(2).diagonal(offset))
...
#         embed = torch.cat((word_embed, tag_embed), -1)
        ...
        return self.embed_dropout(word_embed)
...
if __name__ == "__main__":
    ...
    p.add_argument('--batch-size', default=1, type=int, help='training batch size')
